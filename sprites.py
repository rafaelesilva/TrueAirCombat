...
class Player(pygame.sprite.Sprite):
    def __init__(self, model_name):
        ...
        if self.has_images: self.original_image = self.sprites['neutral']
            ...
            color = (100, 100, 120)
            if self.prefix == 'b2': pygame.draw.polygon(self.original_image, (30, 30, 30), [(base_w//2, 0), (base_w, base_h//2), (base_w//2, base_h-10), (0, base_h//2)])
            elif self.prefix == 'b52': pygame.draw.rect(self.original_image, color, (base_w//2 - 5, 0, 10, base_h)); pygame.draw.rect(self.original_image, color, (0, base_h//3, base_w, 10))
            else: pygame.draw.polygon(self.original_image, color, [(base_w//2, 0), (base_w, base_h), (0, base_h)])
        self.image = self.original_image; self.rect = self.image.get_rect(center=(MAP_WIDTH // 2, MAP_HEIGHT // 2))
        self.hp = self.data['hp']; self.max_hp = self.data['hp']; self.speed = self.data['speed'] * SCALE
        self.weapons = WeaponSystem(self.data.get('loadout', ['VULCAN']))
        self.vel_x, self.vel_y = 0, 0; self.angle = 0; self.is_shooting, self.shoot_timer = False, 0; self.powerup_timer, self.powered_up = 0, False
    def load_and_scale(self, filename):
        path = os.path.join(ASSETS_FOLDER, filename)
        img = pygame.image.load(path).convert_alpha()
        return pygame.transform.scale(img, self.base_size)
    def update_input(self, axis_x, axis_y): self.vel_x = axis_x * self.speed; self.vel_y = axis_y * self.speed
    def trigger_shoot_anim(self): self.is_shooting = True; self.shoot_timer = pygame.time.get_ticks()
    def enable_powerup(self): self.powered_up = True; self.powerup_timer = pygame.time.get_ticks(); self.hp = min(self.hp + 20, self.max_hp)
    def rotate(self):
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, model_name="Gripen F-39E"):
        ...
        loadout = self.data.get('loadout', ['VULCAN']); self.missile_type = loadout[1] if len(loadout) > 1 else loadout[0]
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, center):
        super().__init__()
        self.size = int(60 * SCALE); self.image = pygame.Surface((self.size, self.size), pygame.SRCALPHA); self.rect = self.image.get_rect(center=center); self.timer = 15 
    def update(self):
        self.timer -= 1
        if self.timer % 3 == 0:
            self.image.fill((0,0,0,0)); pygame.draw.circle(self.image, (255, random.randint(100,255), 0), (self.size//2, self.size//2), (self.size//2) * (self.timer/15))
        if self.timer <= 0: self.kill()

class MassiveExplosion(pygame.sprite.Sprite):
    def __init__(self, center):
        super().__init__()
        self.max_size = int(250 * SCALE); self.image = pygame.Surface((self.max_size, self.max_size), pygame.SRCALPHA); self.rect = self.image.get_rect(center=center); self.timer = 0; self.duration = 20
    def update(self):
        self.timer += 1; self.image.fill((0,0,0,0)); progress = self.timer / self.duration; current_radius = int((self.max_size // 2) * progress)
        pygame.draw.circle(self.image, (255, 255, 255, 100), (self.max_size//2, self.max_size//2), current_radius, 5)
        inner_radius = int(current_radius * 0.8); color = (255, random.randint(50, 150), 0)
...
class PowerUp(pygame.sprite.Sprite):
    def __init__(self, center):
        super().__init__()
        self.size = int(25 * SCALE); self.image = pygame.Surface((self.size, self.size)); self.image.fill(CYAN); pygame.draw.rect(self.image, WHITE, (0,0,self.size,self.size), 2)
        font = pygame.font.SysFont('arial', int(15*SCALE), bold=True); txt = font.render("P", True, BLACK)
        self.image.blit(txt, (self.size//2 - txt.get_width()//2, self.size//2 - txt.get_height()//2)); self.rect = self.image.get_rect(center=center)

class Cloud(pygame.sprite.Sprite):
    def __init__(self, x=None, y=None):
        super().__init__()
        w = random.randint(int(100*SCALE), int(300*SCALE)); h = int(w * 0.6); self.image = pygame.Surface((w, h), pygame.SRCALPHA)
